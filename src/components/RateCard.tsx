    ...
    fetch('/api/settings')
      ...
      .then(d => {
        if (d?.contact_whatsapp) {
          setWhatsatsappNumber(d.contact_whatsapp.replace(/\D/g, ''));
        }
      })
      .catch(() => {});
  }, []);

  function getWhatsAppUrl(title: string, price: string) {
    const message = encodeURIComponent(`Hello! I'm interested in your ${title} package (${price}). I'd like to request a quote.`);
